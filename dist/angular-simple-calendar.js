angular.module('500tech.simple-calendar', []).directive('simpleCalendar', function () {
  return {
    ...
    '<div class="day" ng-repeat="date in week  track by $index" ng-class="{default: isDefaultDate(date), event: date.event, disabled: date.disabled || !date, selected: selectedDay == date}" ng-click="onClick(date)">' +
    '<div class="day-number"><div class="text">{{ date.day || "&nbsp;" }}</div><md-progress-circular ng-show="date.event" ng-class="progressClass(date.event.done)" md-mode="determinate" value="{{date.event.done}}"></md-progress-circular></div>' +
